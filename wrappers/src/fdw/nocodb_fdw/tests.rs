...
mod tests {
    ...
    fn nocodb_smoketest() {
        Spi::connect(|mut c| {
            c.update(
                r#"CREATE FOREIGN DATA WRAPPER nocodb_wrapper
                         HANDLER nocodb_fdw_handler VALIDATOR nocodb_fdw_validator"#,
                None,
                None,
            )
            .unwrap();
            c.update(
                r#"CREATE SERVER nocodb_server
                         FOREIGN DATA WRAPPER nocodb_wrapper
                         OPTIONS (
            ...
            c.update(
                r#"
                  CREATE FOREIGN TABLE nocodb_table (
                    bool_field bool,
            ...
            c.update(
                r#"
                  CREATE FOREIGN TABLE nocodb_view (
                    string_field text
                  )
                  SERVER nocodb_server
                  OPTIONS (
            ...
            /*
             The table data below comes from the code in wrappers/dockerfiles/nocodb/server.py
            */
            let results = c
                .select(
                    "SELECT bool_field FROM nocodb_table WHERE bool_field = False",
                    None,
                    None,
                )
            ...
            let results = c
                .select("SELECT string_field FROM nocodb_table", None, None)
                .expect("No results for a given query")
                .filter_map(|r| {
            ...
            let results = c
                .select("SELECT numeric_field FROM nocodb_table", None, None)
                .expect("No results for a given query")
                .filter_map(|r| {
            ...
            let results = c
                .select("SELECT timestamp_field FROM nocodb_table", None, None)
                .expect("No results for a given query")
                .filter_map(|r| {
            ...
            let results = c
                .select("SELECT object_field FROM nocodb_table", None, None)
                .expect("No results for a given query")
                .filter_map(|r| {
            ...
            let results = c
                .select("SELECT strings_array_field FROM nocodb_table", None, None)
                .expect("No results for a given query")
                .filter_map(|r| {
            ...
            let results = c
                .select(
                    "SELECT numerics_array_field FROM nocodb_table",
                    None,
                    None,
                )
                .expect("No results for a given query")
            ...
            let results = c
                .select("SELECT bools_array_field FROM nocodb_table", None, None)
                .expect("No results for a given query")
                .filter_map(|r| {
            ...
            let results = c
                .select("SELECT objects_array_field FROM nocodb_table", None, None)
                .expect("No results for a given query")
                .filter_map(|r| {
            ...
            let results = c
                .select("SELECT string_field FROM nocodb_view", None, None)
                .expect("No results for a given query")
                .filter_map(|r| {
                    r.get_by_name::<&str, _>("string_field")
                        .expect("string_field is missing")
