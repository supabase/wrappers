...
fn to_tokens() -> TokenStream2 {
    quote! {
        ...
        unsafe fn get_operator(opno: pg_sys::Oid) -> pg_sys::Form_pg_operator {
        ...
        unsafe fn unnest_clause(node: *mut pg_sys::Node) -> *mut pg_sys::Node {
        ...
        unsafe fn extract_from_op_expr(
            ...
            if is_a(right, pg_sys::NodeTag_T_Var) && !is_a(left, pg_sys::NodeTag_T_Var) && (*opr).oprcom != 0 {
                let tmp = left;
                left = right;
                right = tmp;
            }
            ...
            if is_a(left, pg_sys::NodeTag_T_Var) && is_a(right, pg_sys::NodeTag_T_Const) {
                ...
                if pg_sys::bms_is_member((*left).varno.try_into().unwrap(), baserel_ids) && (*left).varattno >= 1 {
        ...
        unsafe fn extract_from_null_test(
            baserel_id: pg_sys::Oid,
